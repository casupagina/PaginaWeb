...
function initMap(mapId, data, listId, type) {
  ...
  if (type === "digital" || type === "agua" || type === "alimenticio") {
    ...
    title.innerHTML = isDigital
      ? "Impacto territorial en función al<br>número de parques industriales"
      : isAgua
        ? "Impacto territorial en el nivel de Sequía"
        : "Impacto territorial de la Vulnerabilidad Alimentaria";
    ...
    if (isDigital) {
      ...
      legendDiv.appendChild(
        createBtn("#235f3d", "Vulnerabilidad Alta", "Alta"),
      );
      legendDiv.appendChild(
        createBtn("#526754", "Vulnerabilidad Media", "Media"),
      );
      legendDiv.appendChild(
        createBtn("#84ac9c", "Vulnerabilidad Baja", "Baja"),
      );
      const src = document.createElement("div");
      src.className = "source-text";
    ...
    // Mandamos la leyenda a la columna derecha (sidebar) en lugar del mapa
    let sidebar = document.querySelector(`#${type} .geo-sidebar`);
    if (sidebar) {
